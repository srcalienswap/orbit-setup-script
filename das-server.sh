...
if jq -e '.node | has("data-availability")' /home/user/.arbitrum/nodeConfig.json >/dev/null; then
    ...
    --enable-rpc --rpc-addr '0.0.0.0' \
    --enable-rest --rest-addr '0.0.0.0' \
    --log-level 3 \
...
# Archive DA server config, local-file-storage and ipfs-storage don't discard data after expiring, so the option discard-after-timeout is not available.
